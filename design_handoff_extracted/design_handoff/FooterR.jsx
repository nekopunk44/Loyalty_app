function FooterR() {
  const nav = [{href:'#rooms',label:'Номера'},{href:'#tour',label:'3D-тур'},{href:'#loyalty',label:'Лояльность'},{href:'#reviews',label:'Отзывы'},{href:'#faq',label:'FAQ'},{href:'#contact',label:'Бронирование'}];
  const social = [{name:'Instagram',icon:'camera'},{name:'Telegram',icon:'send'},{name:'WhatsApp',icon:'message-circle'}];
  const year = new Date().getFullYear();

  const linkStyle = { fontSize:14, color:'var(--r-text-soft)', textDecoration:'none', transition:'color 0.3s ease' };
          ...
          <div>
            <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:16 }}>
              <svg viewBox="0 0 100 100" fill="none" width="36" height="36" style={{ color:'var(--r-gold)', flexShrink:0 }}>
                <g stroke="currentColor" strokeWidth="0.5" opacity="0.3"><line x1="14" y1="12" x2="14" y2="88"/><line x1="36.6" y1="12" x2="36.6" y2="88"/><line x1="50" y1="12" x2="50" y2="88"/><line x1="63.4" y1="12" x2="63.4" y2="88"/><line x1="86" y1="12" x2="86" y2="88"/><line x1="12" y1="14" x2="88" y2="14"/><line x1="12" y1="36.6" x2="88" y2="36.6"/><line x1="12" y1="50" x2="88" y2="50"/><line x1="12" y1="63.4" x2="88" y2="63.4"/><line x1="12" y1="86" x2="88" y2="86"/></g>
        ...
        <div style={{ display:'grid', gap:48, gridTemplateColumns:'repeat(auto-fit,minmax(min(180px,100%),1fr))', marginBottom:64 }}>
          <div>
            ...
            <ul style={{ listStyle:'none', padding:0, margin:0, display:'flex', flexDirection:'column', gap:12 }}>
              {nav.map(l=>(
                <li key={l.href}>
                  <a href={l.href} style={{ ...linkStyle, color:'rgba(247,242,232,0.6)' }}
                    onMouseEnter={e=>e.currentTarget.style.color='var(--r-gold)'}
                    onMouseLeave={e=>e.currentTarget.style.color='rgba(247,242,232,0.6)'}>{l.label}</a>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <ul style={{ listStyle:'none', padding:0, margin:0, display:'flex', flexDirection:'column', gap:14 }}>
              <li><a href="tel:+37377812345" style={{ display:'flex', alignItems:'center', gap:9, ...linkStyle, color:'rgba(247,242,232,0.6)' }} onMouseEnter={e=>e.currentTarget.style.color='var(--r-gold)'} onMouseLeave={e=>e.currentTarget.style.color='rgba(247,242,232,0.6)'}><Icon name="phone" size={13} strokeWidth={1.5} />+373 778 12 345</a></li>
              <li><a href="mailto:hello@villajaconda.com" style={{ display:'flex', alignItems:'center', gap:9, ...linkStyle, color:'rgba(247,242,232,0.6)' }} onMouseEnter={e=>e.currentTarget.style.color='var(--r-gold)'} onMouseLeave={e=>e.currentTarget.style.color='rgba(247,242,232,0.6)'}><Icon name="mail" size={13} strokeWidth={1.5} />hello@villajaconda.com</a></li>
          ...
          <div>
            ...
            <ul style={{ listStyle:'none', padding:0, margin:0, display:'flex', flexDirection:'column', gap:12 }}>
              {social.map(s=>(
                <li key={s.name}>
                  <a href="#" style={{ display:'flex', alignItems:'center', gap:9, ...linkStyle, color:'rgba(247,242,232,0.6)' }}
                    onMouseEnter={e=>e.currentTarget.style.color='var(--r-gold)'}
