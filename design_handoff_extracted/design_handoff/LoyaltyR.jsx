function LoyaltyR() {
  const tiers = [
    { key:'bronze', name:'Bronze', threshold:0, next:500, color:'#E08B32', cashback:'1%',
      perks:['Кешбек 1% с каждого бронирования','Бонус в день рождения','Полный архив визитов'] },
    { key:'silver', name:'Silver', threshold:500, next:2000, color:'#7A90A8', cashback:'1.5%',
      perks:['Кешбек 1.5% — +0.5% к базовому','Приоритетная поддержка','Бонус ко дню рождения ×2'] },
    { key:'gold', name:'Gold', threshold:2000, next:5000, color:'#C08828', cashback:'2%',
      perks:['Кешбек 2% — двойной бонус','Ранний доступ к акциям и событиям','VIP поддержка 24/7'] },
    { key:'platinum', name:'Platinum', threshold:5000, next:null, color:'#9060CC', cashback:'3%',
      perks:['Кешбек 3% — максимальный','Личный менеджер всегда на связи','Закрытые события и тарифы'] },
  ];
              ...
              return (
                <button key={tier.key} onClick={() => setActive(i)}
                  ...
                  <div style={{
                    ...
                    borderRadius:'50%',
                    background: isActive || isPast ? tier.color : 'var(--r-bg)',
                    border: `2px solid ${isActive || isPast ? tier.color : 'var(--r-line-strong)'}`,
                    boxShadow: isActive ? `0 0 0 6px ${tier.color}22, 0 8px 24px ${tier.color}44` : 'none',
                  ...
                  <div style={{ textAlign:'center' }}>
                    <p style={{ fontFamily:'var(--r-serif)', fontSize: isActive ? 22 : 16, fontWeight: isActive ? 400 : 300, color: isActive ? tier.color : 'var(--r-muted)', lineHeight:1, marginBottom:6, transition:'all 0.3s ease' }}>
                      {tier.name}
                    </p>
                    <p style={{ fontSize:10, color:'var(--r-muted)', letterSpacing:'0.06em', fontVariantNumeric:'tabular-nums' }}>
                      {tier.next ? `${tier.threshold}–${tier.next}` : `${tier.threshold}+`} балл.
                    </p>
                  </div>
                </button>
              );
          ...
          <div style={{ background:`${t.color}10`, borderRadius:12, padding:'clamp(32px,4vw,48px)', border:`1px solid ${t.color}30`, display:'flex', flexDirection:'column', justifyContent:'space-between' }}>
            <div>
              ...
              <p style={{ fontSize:13, color:'var(--r-text-soft)', lineHeight:1.65 }}>
                Баллы начисляются за каждую ночь проживания. {tiers[active+1] ? `До следующего уровня (${tiers[active+1].name}) — ${tiers[active+1].threshold - tiers[active].threshold} баллов.` : 'Вы на максимальном уровне!'}
              </p>
            </div>
            <a href="#contact" onClick={e => { e.preventDefault(); document.getElementById('contact')?.scrollIntoView(); }}
              style={{ marginTop:32, display:'inline-flex', alignItems:'center', gap:10, padding:'14px 24px', background:t.color, color:'#fff', borderRadius:999, fontSize:12, letterSpacing:'0.14em', textTransform:'uppercase', textDecoration:'none', fontWeight:500, transition:'opacity 0.3s ease', alignSelf:'flex-start' }}
