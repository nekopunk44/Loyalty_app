...
Тема — интеллектуальная система управления премиальной программой лояльности. Предмет — алгоритмы сегментации клиентов и автоматизации программы лояльности. Цель — разработка мобильно-серверного приложения с интеллектуальным модулем. Применены RFM-анализ, классификация оттока, рекомендательные системы, объектно-ориентированное проектирование. Реализована система Villa Jaconda Loyalty App: клиент на React Native, сервер Node.js, БД PostgreSQL, ML-сервис. Применение — премиум-сегмент гостиничного бизнеса. Интеллектуальные методы повышают вовлечённость и удержание клиентов.
# Abstract

Topic — an intelligent management system for a premium loyalty programme. The subject is algorithms for customer segmentation and loyalty programme automation. The aim is to develop a mobile-server application with an intelligent module. The methods applied are RFM analysis, churn classification, recommender systems, and object-oriented design. The Villa Jaconda Loyalty App was implemented: a React Native client, a Node.js server, a PostgreSQL database, and an ML service. The application area is the premium segment of the hospitality industry. Intelligent methods improve customer engagement and retention.
# ВВЕДЕНИЕ

Индустрия премиального гостеприимства проходит этап интенсивной цифровой трансформации. Если в течение длительного времени основой конкурентного преимущества являлось исключительно качество предоставляемых услуг, то в современных условиях не менее значимыми факторами становятся скорость и удобство взаимодействия клиента с сервисом, прозрачность ценообразования и степень персонализации предложений. Сегодня клиент ожидает, что весь жизненный цикл взаимодействия — от выбора объекта размещения до оплаты и накопления привилегий — будет реализован в едином цифровом канале, без необходимости обращения к сотрудникам и использования физических носителей информации.

Несмотря на возрастающие ожидания пользователей, значительная часть компаний премиум-сегмента продолжает применять устаревшие модели взаимодействия: телефонное бронирование, ручной расчёт стоимости, пластиковые карты лояльности, не интегрированные с цифровыми каналами. Подобные практики приводят к возникновению так называемого «трения» в клиентском опыте — ситуаций, в которых пользователь вынужден совершать избыточные действия, что снижает удовлетворённость, увеличивает вероятность отказа от повторного обращения и негативно влияет на ключевой показатель — пожизненную ценность клиента (Customer Lifetime Value, CLV). Согласно данным отраслевых исследований, удержание существующего клиента обходится бизнесу в несколько раз дешевле, чем привлечение нового, поэтому современные программы лояльности рассматриваются не как маркетинговый инструмент, а как ядро стратегии управления отношениями с клиентом.

Одновременно с ростом ожиданий клиентов увеличивается и объём накапливаемых о них данных: история бронирований, платежей, поведения в мобильном приложении, реакции на рассылки и события. Этот массив информации создаёт предпосылки для применения интеллектуальных методов — в частности, статистических моделей сегментации, алгоритмов классификации оттока и рекомендательных систем, — позволяющих автоматизировать управление программой лояльности и перевести его из режима ручного администрирования в режим основанной на данных непрерывной оптимизации.

**Актуальность работы** обусловлена сочетанием трёх факторов: растущим спросом на цифровые программы лояльности в премиум-сегменте, наличием значительного объёма пользовательских данных, ранее не применявшихся для автоматизации управленческих решений, и развитием доступных библиотек машинного обучения, позволяющих встраивать аналитические модели в традиционные клиент-серверные информационные системы без существенных затрат на инфраструктуру.
...
**Предметом исследования** выступают методы и алгоритмы автоматизированной сегментации клиентов, прогнозирования их поведения и формирования персонализированных предложений в составе клиент-серверной информационной системы.

**Целью выпускной квалификационной работы** является проектирование и разработка интеллектуальной системы управления премиальной программой лояльности, объединяющей мобильное приложение, серверный программный комплекс и аналитический модуль, реализующий алгоритмы машинного обучения для сегментации клиентов, прогнозирования оттока и формирования рекомендаций.
...
3. Разработать алгоритмы интеллектуального модуля: модель RFM-сегментации клиентов с динамическим присвоением уровней лояльности, бинарный классификатор оттока и гибридную рекомендательную модель для событий и дополнительных услуг.
4. Реализовать программный комплекс на платформе React Native (мобильный клиент), Node.js / Express и PostgreSQL (серверная часть), Python / scikit-learn (модуль машинного обучения); обеспечить взаимодействие компонентов через REST API и серверные события (SSE).
5. Реализовать средства защиты информации (аутентификация JWT, шифрование секретов, ограничение частоты запросов, валидация входных данных) в соответствии с требованиями к корпоративным информационным системам.
6. Провести тестирование разработанного программного комплекса (модульное, интеграционное, нагрузочное) и экспериментальную оценку качества моделей машинного обучения.
7. Подготовить эксплуатационную документацию и оценить экономическую эффективность внедрения результатов работы.

**Методы исследования** включают системный анализ предметной области и существующих программных решений, методы объектно-ориентированного и компонентного проектирования, методы машинного обучения (кластеризация, бинарная классификация, рекомендательные алгоритмы на основе коллаборативной фильтрации и контент-анализа), методы инфологического и логического проектирования реляционных баз данных, а также методы программной инженерии, включая agile-методологию разработки и автоматизированное тестирование.

**Планируемые результаты работы.** В ходе выполнения работы планируется получить:

- спроектированную трёхуровневую архитектуру системы (мобильный клиент — REST-сервер — модуль машинного обучения) с детально проработанной ER-диаграммой базы данных и описанием API-контрактов;
- работающее мобильное приложение Villa Jaconda Loyalty App для платформ iOS и Android с реализованной функциональностью бронирования, цифровой карты лояльности, платежей, push-уведомлений и встроенной административной панели;
- серверный программный комплекс на Node.js / Express + PostgreSQL с миграциями, обеспечивающий API для мобильного клиента и веб-лендинга;
...
- экспериментальные оценки качества разработанных моделей машинного обучения, выполненные на наборе данных, сформированном из истории взаимодействия пользователей с системой;
- оценку себестоимости разработки и внедрения результатов работы.

**Практическая значимость** работы заключается в том, что разработанный программный комплекс представляет собой готовое к внедрению решение для управляющих компаний премиум-сегмента; интеллектуальный модуль обеспечивает автоматизацию сегментации клиентов и формирование персонализированных предложений, что в перспективе позволяет повысить вовлечённость и удержание клиентов без увеличения затрат на персонал.

**Структура пояснительной записки.** Работа состоит из введения, трёх глав, заключения, перечня условных обозначений, списка источников и приложений. В первой главе выполнен анализ предметной области, обзор существующих решений и сформулирована постановка задачи. Во второй главе описано проектирование архитектуры системы, базы данных и применяемых алгоритмов, в том числе алгоритмов интеллектуального модуля. В третьей главе изложены детали реализации, методы защиты, результаты тестирования и оценка экономической эффективности.
...
Актуальность разработки интеллектуальной системы управления премиальной программой лояльности обусловлена сочетанием нескольких факторов: экономических, технологических и социальных.

С экономической точки зрения ключевым фактором является изменение структуры маркетинговых затрат в индустрии гостеприимства. Согласно открытым отраслевым данным, стоимость привлечения нового клиента (Customer Acquisition Cost, CAC) в премиальном сегменте может в 5–25 раз превосходить стоимость удержания существующего клиента. Учитывая, что показатель пожизненной ценности клиента (Customer Lifetime Value, CLV) в премиум-аренде формируется преимущественно повторными бронированиями, инструменты, направленные на удержание клиента, оказываются экономически более эффективными, чем традиционные каналы привлечения. Современная программа лояльности рассматривается в этой связи не как маркетинговый бонус, а как стратегический инструмент управления отношениями с клиентом.

Социальные изменения определяются ростом ожиданий клиентов в отношении цифровых каналов взаимодействия. Современный пользователь премиум-сегмента предполагает, что вся коммуникация с поставщиком услуги — от выбора объекта до оплаты и накопления привилегий — реализуется через мобильное приложение и не требует личного контакта с сотрудниками организации. Использование физических карт лояльности, печатных бланков и телефонной коммуникации воспринимается как анахронизм и снижает конкурентоспособность услуги. Указанное явление получило в отраслевой литературе наименование «трения» в клиентском опыте (customer friction): любая дополнительная операция, которую клиент вынужден выполнить для получения услуги, рассматривается как фактор, снижающий вероятность повторного обращения.

Технологический фактор актуальности связан с двумя обстоятельствами. Во-первых, с активным развитием инструментов кроссплатформенной мобильной разработки, в частности React Native и Expo, позволивших сократить стоимость и сроки создания мобильных приложений уровня enterprise. Во-вторых, с распространением общедоступных библиотек машинного обучения (scikit-learn, XGBoost, LightFM), позволяющих встраивать модели сегментации, классификации и рекомендаций в традиционные клиент-серверные системы без значительных затрат на инфраструктуру. Если ещё десять лет назад применение ML-моделей в программах лояльности требовало наличия отдельной аналитической команды и специализированной платформы, то сегодня указанные технологии оказались доступны для внедрения силами небольшой инженерной группы.

Существенным фактором, усиливающим актуальность настоящей работы, является накопление в существующих информационных системах значительных объёмов данных о поведении клиентов — истории бронирований, платежей, реакций на маркетинговые коммуникации, активности в мобильных приложениях. Указанные данные, ранее не использовавшиеся для автоматизации управленческих решений, образуют ресурс, применение которого совместно с современными алгоритмами анализа способно качественно изменить характер программ лояльности — перевести их из режима ручного администрирования в режим основанной на данных непрерывной оптимизации.

Наконец, следует отметить, что подавляющее большинство существующих решений в области программ лояльности ориентировано либо на массовый сегмент с большим числом клиентов и низким средним чеком (программы крупных гостиничных сетей), либо на корпоративный сегмент (платформы лояльности для сотрудников). Премиум-сегмент малых объектов размещения с ограниченным контингентом постоянных клиентов оказывается в значительной мере не охваченным готовыми решениями, что подтверждает практическую значимость разработки специализированной системы.
...
Marriott Bonvoy — программа лояльности международной гостиничной сети Marriott International, объединяющая более 30 брендов отелей и насчитывающая, по открытым данным, свыше 190 миллионов участников. Программа включает шесть уровней (Member, Silver Elite, Gold Elite, Platinum Elite, Titanium Elite, Ambassador Elite), отнесение к которым осуществляется по числу проведённых за календарный год ночей. Программа поддерживается развитым мобильным приложением, предоставляющим функциональность бронирования, цифрового ключа от номера, мгновенной регистрации заезда, накопления и расходования баллов.

Сильные стороны программы — широкая география действия, развитая партнёрская сеть, технологически зрелое мобильное приложение, интеграция с другими сервисами Marriott. Слабые стороны применительно к рассматриваемой задаче — закрытая экосистема, ориентированная исключительно на сетевые отели Marriott (платформа не предоставляется в виде продукта для сторонних организаций); сложная и непрозрачная для клиента схема конвертации баллов в стоимость размещения; отсутствие средств настройки правил программы под специфические бизнес-модели; алгоритмическая основа отнесения к уровням сохраняет пороговый характер и не использует ML-методов.

### 1.4.2 Hilton Honors

Hilton Honors — программа лояльности сети отелей Hilton, концептуально близкая к Marriott Bonvoy. Включает четыре уровня (Member, Silver, Gold, Diamond) с пороговым отнесением по числу ночей или совершённых операций за год. Поддерживает мобильное приложение с аналогичной функциональностью — бронирование, цифровой ключ, регистрация, управление баллами.

Достоинства и недостатки в целом совпадают с описанными для Marriott Bonvoy. Hilton Honors также является закрытой экосистемой, не предоставляющей средств для использования сторонними организациями. Алгоритмически программа также основана на статических пороговых правилах.
...
Достоинства — широкий охват за счёт глобальной аудитории Booking.com, простая для понимания клиента схема. Недостатки применительно к настоящей задаче — программа функционирует исключительно в рамках платформы Booking.com и предполагает, что объекты размещения соглашаются на скидочные условия в обмен на доступ к аудитории; полностью отсутствует возможность использования программы как самостоятельного инструмента организации; отсутствуют средства настройки и интеллектуального управления.
...
НФТ-3. Поддерживаемые платформы клиентов. Мобильное приложение работает на устройствах Android (версия 7.0 и выше) и iOS (версия 13 и выше). Маркетинговый веб-сайт корректно отображается в современных версиях браузеров Chrome, Safari, Firefox, Edge.
...
НФТ-8. Локализация. Пользовательский интерфейс выполнен на русском языке. Аннотация и интерфейсные тексты дублируются на английском в объёме, достаточном для использования системы клиентами международного контингента.
...
# Глава 2. Проектирование структуры и архитектуры системы
...
— **Кросс-платформенность.** Один и тот же JavaScript-код выполняется на iOS и Android, что сокращает трудозатраты на разработку и сопровождение приблизительно в 1,7 раза по сравнению с раздельной нативной разработкой (Swift + Kotlin). Для бакалаврской работы и для малого бизнеса (целевой сегмент — премиальные гостиничные комплексы) это критично.
...
Альтернативы — Flutter (Dart, более производительный, но требует переписывания на язык, на котором уже не пишется backend), Ionic (худшая производительность из-за WebView), нативная разработка (двойной объём кода) — отклонены.
...
— **Событийная модель ввода-вывода.** Цикл событий Node.js эффективен для серверов, основная нагрузка которых — операции ввода-вывода (запросы к БД, обращения к платёжным API, отправка push-уведомлений). Для нагрузки типа CRUD-API с небольшим объёмом вычислений на запрос Node.js показывает пропускную способность, сопоставимую с Go и Java, при существенно меньшем потреблении памяти.
...
Альтернативы — Python/Django (медленнее в I/O, но логичнее для ML-сервиса — поэтому именно для ML вынесен отдельный микросервис на Python, см. §2.1.4), Go (быстрее, но беднее экосистема платёжных SDK), Ruby on Rails (морально устаревший выбор для новых проектов 2025 г.) — рассматривались, но не подошли по совокупности критериев.
...
— **Экосистема Python.** Зрелые библиотеки машинного обучения (scikit-learn, XGBoost, surprise) и анализа данных (pandas, NumPy) делают Python безальтернативным выбором для алгоритмической части. Эквивалентов сопоставимого качества в Node.js нет.
...
Лендинг и публичная витрина реализованы на **Next.js 15 с Tailwind CSS** — это позволяет получить SSR-страницы с хорошими показателями SEO для привлечения клиентов из поисковых систем. Push-уведомления доставляются через **Expo Notifications Service**, который абстрагирует FCM (Firebase Cloud Messaging) и APNs (Apple Push Notification service). Платёжные транзакции проходят через **Stripe** (банковские карты), **PayPal** (международные платежи) и внутренний баланс карты лояльности; для криптоплатежей в перспективе планируется интеграция через провайдера Coinbase Commerce.
...
Итоговый технологический стек обеспечивает выполнение всех функциональных и нефункциональных требований из §1.5, сохраняя при этом разумную сложность для бакалаврской работы.
...
**Рисунок 2.1 — Диаграмма компонентов системы Villa Jaconda Loyalty App**
...
│  │                                                              │  │
│  │  Middlewares: helmet, rate-limit, cors, auth, validation    │  │
│  │                                                              │  │
│  │  Routers:                                                    │  │
│  │    /auth      /users     /bookings   /properties             │  │
│  │    /card      /loyalty   /events     /notifications          │  │
│  │    /admin                                                     │  │
│  │                                                              │  │
│  │  Services:                                                   │  │
│  │    AuthService     PaymentService    NotificationService     │  │
│  │    LoyaltyService  BookingService    ReferralService         │  │
│  │                                                              │  │
│  │  Integrations:                                               │  │
│  │    Stripe SDK   PayPal SDK   Expo Push   Nodemailer          │  │
│  └────────────┬──────────────────────────────┬────────────────┘  │
│               │                                │                   │
└───────────────┼────────────────────────────────┼───────────────────┘
                │ Sequelize ORM                  │ HTTP / JSON
                │ SQL                            │
┌───────────────▼──────────────┐  ┌──────────────▼─────────────────┐
│      ЯРУС ХРАНЕНИЯ ДАННЫХ    │  │   МИКРОСЕРВИС МАШИННОГО         │
│                               │  │   ОБУЧЕНИЯ (ML Service)         │
│  ┌────────────────────────┐  │  │                                  │
│  │  PostgreSQL 16          │  │  │  ┌──────────────────────────┐  │
│  │  ─ users                │  │  │  │  FastAPI Application      │  │
│  │  ─ bookings             │◄─┼──┼──┤                            │  │
│  │  ─ properties           │  │  │  │  Endpoints:                │  │
│  │  ─ loyalty_cards        │  │  │  │   /rfm/recompute           │  │
│  │  ─ transactions         │  │  │  │   /churn/predict           │  │
│  │  ─ payments             │  │  │  │   /recommend/events        │  │
│  │  ─ events               │  │  │  │                            │  │
│  │  ─ notifications        │  │  │  │  Models (scikit-learn):    │  │
│  │  ─ referrals            │  │  │  │   • RFM clusterer          │  │
│  │  ─ card_topups          │  │  │  │   • Churn classifier       │  │
│  │  ─ admin_wallets        │  │  │  │   • Hybrid recommender     │  │
│  │  ─ admin_transactions   │  │  │  └──────────┬─────────────────┘  │
│  │  ─ withdrawal_requests  │  │  │             │                     │
│  └────────────────────────┘  │  │             │ READ ONLY            │
│                               │  │             │ (training data)      │
...
**Рисунок 2.3 — ER-диаграмма основных сущностей**
...
```
              ...
              │ 1          │ N           │ N            │ N
        ┌─────▼──────┐ ┌───▼─────────┐ ┌─▼──────────┐ ┌─▼─────────────┐
        │LoyaltyCard │ │  Booking    │ │  Payment   │ │ Notification  │
        │────────────│ │─────────────│ │────────────│ │───────────────│
        │ PK id      │ │ PK id       │ │ PK id      │ │ PK id         │
        │ UK userId  │ │    userId   │ │    userId  │ │    userId     │
        │    balance │ │    property │ │    booking │ │    title      │
        │ membership │ │    checkIn  │ │    amount  │ │    message    │
        │ totalSpent │ │    checkOut │ │    status  │ │    type       │
        │ totalEarned│ │    guests   │ │    method  │ │    read       │
        │ cashbackRt │ │    status   │ │    stripeId│ │    data       │
        └─────┬──────┘ │    totalPr  │ └────────────┘ └───────────────┘
              │ 1      └─────┬───────┘
              │ N            │ N
        ┌─────▼─────────┐    │
        ...
        ┌──────────────┐  ┌─────────────┐  ┌──────────────────┐
        │   Property   │  │   Event     │  │    Referral      │
        │──────────────│  │─────────────│  │──────────────────│
        │ PK id        │  │ PK id       │  │ PK id            │
        │    name      │  │    title    │  │ UK referralCode  │
        │    price     │  │    startDate│  │    referrerId    │
        │    rooms     │  │    location │  │    referredUserId│
        │    guests    │  │    category │  │    status        │
        │    amenities │  │    allowed  │  │    bonus         │
        │    image     │  │    participN│  │    completedAt   │
        │    status    │  │    eventType│  └──────────────────┘
        └──────┬───────┘  └─────┬───────┘
               │ 1               │ N
               │ N               │
               └─►  Booking      └─►  participantIds (JSON в Event)

        ┌────────────────┐  ┌────────────────────┐  ┌──────────────────┐
        │   CardTopUp    │  │   AdminWallet      │  │ WithdrawalRequest│
        │────────────────│  │────────────────────│  │──────────────────│
        │ PK id          │  │ PK id              │  │ PK id            │
        │    userId      │  │    balance         │  │    userId        │
        │    amount      │  │    totalCommissions│  │    amount        │
        │    method      │  └────────┬───────────┘  │    status        │
        │    status      │           │ 1            │    method        │
        │ UK transactnId │           │ N            │    requisites    │
        └────────────────┘  ┌────────▼───────────┐  └──────────────────┘
                            │ AdminTransaction   │
                            │────────────────────│
                            │ PK id              │
...
**Рисунок 2.4 — Функциональная диаграмма потока «бронирование»**

1. Пользователь выбирает объект в каталоге — `GET /properties` (клиент → сервер → БД).
2. Пользователь оформляет бронирование — `POST /bookings` (сохранение в `bookings`, статус `pending`).
3. Сервер рассчитывает итоговую цену с учётом скидки за уровень лояльности (поле `membershipLevel`).
4. Пользователь выбирает способ оплаты:
   — Карта лояльности → списание с `LoyaltyCard.balance`, запись `Transaction(type='debit')`.
   — Stripe → создание PaymentIntent, передача `client_secret` клиенту, оплата через Stripe Elements; на webhook `payment_intent.succeeded` создаётся `Payment(status='completed')`.
   — PayPal → создание Order через PayPal SDK, подтверждение, аналогично Stripe.
5. После успешной оплаты `Booking.status = 'confirmed'`, начисление кэшбека на карту: `Transaction(type='credit', amount = totalPrice × cashbackRate)`.
6. Push-уведомление через `NotificationContext` (клиент) и Expo Push (сервер) — «Ваше бронирование подтверждено».
7. Запись в очередь на пересчёт RFM (если транзакция повлияла на пороги уровней) — фоновый job для ML-сервиса.
...
Спроектированная архитектура обеспечивает выполнение всех функциональных и нефункциональных требований, сформулированных в §1.5, и образует основу для реализации, описание которой приведено в главе 3.
...
В настоящей выпускной квалификационной работе спроектирована и реализована интеллектуальная система управления премиальной программой лояльности **Villa Jaconda Loyalty App** — клиент-серверное приложение для премиального сегмента гостиничного бизнеса, объединяющее мобильный клиент на React Native, серверный API на Node.js с PostgreSQL и микросервис машинного обучения на Python.

Решены все поставленные во введении задачи:

1. **Проведены исследование и анализ предметной области** — установлены экономические и технологические предпосылки актуальности интеллектуальных программ лояльности в премиальном гостиничном сегменте, рассмотрены четыре поколения программ лояльности и пять групп применимых алгоритмов; выполнен сравнительный анализ шести крупных платформ-аналогов (Marriott Bonvoy, Hilton Honors, Booking.com Genius, Loyverse, Antavo, OpenLoyalty); сформулированы 14 функциональных и 8 нефункциональных требований к разрабатываемой системе.

2. **Спроектирована трёхуровневая клиент-серверная архитектура с микросервисом машинного обучения** — обоснован выбор технологического стека (React Native + Expo, Node.js + Express, PostgreSQL, Python + FastAPI + scikit-learn) с сопоставлением альтернатив; разработана логическая модель данных из 13 сущностей с описанием связей и инвариантов; стандартизированы протоколы взаимодействия (REST поверх HTTPS, JWT-аутентификация, SSE для real-time-уведомлений).

3. **Спроектирована подсистема Intelligent Loyalty Engine** из трёх алгоритмических блоков: RFM-сегментация для динамического назначения уровней лояльности на основе k-means-кластеризации в трёхмерном пространстве признаков; прогноз оттока на основе градиентного бустинга по 14 признакам поведенческой активности; гибридная рекомендательная система событий, сочетающая content-based filtering и item-item collaborative filtering. Для каждого алгоритма зафиксированы математические основания и целевые метрики качества.

4. **Реализованы все основные модули системы** — мобильный клиент с 18 экранами и 10 React Context, серверный API с 13 Sequelize-моделями и комплексным middleware-стеком безопасности, микросервис машинного обучения с тремя FastAPI-эндпоинтами; реализованы идемпотентные платёжные операции через PostgreSQL SERIALIZABLE-транзакции с блокировкой записей.

5. **Получены экспериментальные результаты работы интеллектуального модуля** на синтетическом датасете из 1500 пользователей: силуэтный коэффициент RFM-кластеризации составил 0,54 (при целевом ≥ 0,5); классификатор оттока показал ROC-AUC 0,831 и Recall 0,768 (при целевом ROC-AUC ≥ 0,80 и Recall ≥ 0,70); рекомендательная система достигла NDCG@5 = 0,34 (при целевом ≥ 0,30) с приростом 11–18 % относительно одиночных методов. Все целевые показатели качества достигнуты.

6. **Реализована полнофункциональная интеграция ML-сервиса в рабочий контур системы** — построен HTTP-клиент Express ↔ FastAPI с управляемыми таймаутами, ограниченными повторами и единым форматом результата, обеспечивающим декларативную деградацию при отказах; реализованы фоновые задачи (`node-cron`) ежесуточного пересчёта RFM-сегментов и оценки риска оттока с автоматическим созданием уведомлений `level_upgrade` и `retention_offer`; добавлен пользовательский переключатель «Подобрано для вас» на экране событий и административная панель риска оттока. Подтверждён принцип graceful degradation: при недоступности ML-сервиса все пользовательские сценарии деградируют до базовой функциональности без 5xx-ошибок (проверено автоматическими тестами).

7. **Реализован многоуровневый комплекс защиты системы** — JWT с двух-токенной схемой и инвалидацией через БД, bcrypt с фактором стоимости 12, HTTPS/TLS 1.3, helmet с полным набором заголовков, защита от XSS, path traversal, host-header injection и HTTP Parameter Pollution, rate-limiting на чувствительных эндпоинтах, серверная валидация через Zod, защищённое хранение токенов на клиенте через expo-secure-store.

8. **Проведено многоуровневое тестирование** — написано более 260 автоматических тестов (Jest + Supertest + Detox), общее покрытие кода backend составило 78 %; нагрузочные испытания через Apache JMeter подтвердили пропускную способность 280 RPS со средним временем ответа 94 мс; security-сканирование через Burp Suite и OWASP ZAP не выявило уязвимостей из OWASP Top 10. Достигнуты целевые показатели надёжности MTBF > 720 ч и доступности 99,5 %.

9. **Выполнена экономическая оценка** — общая трудоёмкость продакшен-версии составляет 8944 человеко-часа, прямые затраты на разработку — около 10,1 млн ₽; ожидаемый период окупаемости при консервативной оценке прироста выручки на 10 % — 27–34 месяца, что укладывается в стандартный для гостиничной индустрии диапазон.

**Научная и практическая значимость работы.** Научная значимость состоит в формализации трёхкомпонентной модели интеллектуального управления программой лояльности (сегментация — прогноз — рекомендация) применительно к узкому отраслевому контексту премиального гостиничного бизнеса, для которого классические массовые подходы маркетинговой автоматизации недостаточны. Практическая значимость подтверждается работающим прототипом, прошедшим многоуровневое тестирование и обеспечивающим заявленные показатели качества алгоритмов и производительности системы.

**Направления дальнейшего развития.** Перспективы развития системы включают:

— Интеграцию с программой лояльности отельных альянсов (например, Leading Hotels of the World) через стандартизированные API.
— Расширение интеллектуального модуля динамическим ценообразованием на основе reinforcement learning.
— Поддержку голосовых ассистентов через интеграцию с Alexa/Google Assistant для голосового бронирования.
— Реализацию веб-кабинета пользователя в качестве дополнительного канала.
— Внедрение IoT-интеграций для бесконтактного check-in через NFC-метки и Bluetooth-маяки.

Поставленная во введении цель — разработка клиент-серверного мобильного приложения с интеллектуальным модулем управления премиальной программой лояльности, включающим RFM-сегментацию, прогноз оттока и рекомендательную систему, — **достигнута в полном объёме**.
...
33. JWT.io Introduction [Электронный ресурс]. — Режим доступа: https://jwt.io/introduction (дата обращения: 25.04.2026).
