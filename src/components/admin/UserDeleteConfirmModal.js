...
export default function UserDeleteConfirmModal({ visible, onClose, user, theme, onUserDeleted, onNotify }) {
  ...
  const handleDeleteUser = async () => {
    setLoading(true);
    try {
      const response = await apiCall(API_ENDPOINTS.USERS.DELETE(user.id), { method: 'DELETE' });
      if (!response.success) throw new Error(response.error || 'Не удалось удалить пользователя');
