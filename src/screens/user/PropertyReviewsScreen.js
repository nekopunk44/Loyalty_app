...
export default function PropertyReviewsScreen({ route, navigation }) {
  ...
  const loadReviews = async () => {
    try {
      const data = await apiCall(`${getApiUrl()}/reviews/property/${propertyId}?filter=approved`);
      if (!data.error) {
        setStats(data.stats);
  ...
  const handleSubmitReview = async () => {
    ...
    try {
      if (editingReview) {
        ...
        const result = await apiCall(`${getApiUrl()}/reviews`, {
          method: 'POST',
          body: JSON.stringify({
            propertyId,
        ...
        if (result.error) {
          throw new Error(result.error || 'Ошибка при создании отзыва');
        }
